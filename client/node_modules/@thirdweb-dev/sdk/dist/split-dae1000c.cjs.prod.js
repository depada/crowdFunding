...
class Split {
  ...
  constructor(network, address, storage) {
     ...
     */
    QueryParams._defineProperty(this, "withdraw", contractMetadata.buildTransactionFunction(async walletAddress => {
      return contractMetadata.Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "release(address)",
        args: [await contractMetadata.resolveAddress(walletAddress)]
      });
    }));
    /**
     ...
     */
    QueryParams._defineProperty(this, "withdrawToken", contractMetadata.buildTransactionFunction(async (walletAddress, tokenAddress) => {
      return contractMetadata.Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "release(address,address)",
        args: [await contractMetadata.resolveAddress(tokenAddress), await contractMetadata.resolveAddress(walletAddress)]
      });
    }));
    /**
     * Distribute Funds
     ...
     * ```
     */
    QueryParams._defineProperty(this, "distribute", contractMetadata.buildTransactionFunction(async () => {
      return contractMetadata.Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "distribute()",
        args: []
     ...
     * @param tokenAddress - The address of the currency contract to distribute funds
     */
    QueryParams._defineProperty(this, "distributeToken", contractMetadata.buildTransactionFunction(async tokenAddress => {
      return contractMetadata.Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "distribute(address)",
        args: [await contractMetadata.resolveAddress(tokenAddress)]
      });
    }));
    ...
    this.metadata = new contractMetadata.ContractMetadata(this.contractWrapper, contractMetadata.SplitsContractSchema, this.storage);
    this.app = new contractMetadata.ContractAppURI(this.contractWrapper, this.metadata, this.storage);
    this.roles = new contractMetadata.ContractRoles(this.contractWrapper, Split.contractRoles);
  ...
  async balanceOfTokenAllRecipients(tokenAddress) {
    const resolvedToken = await contractMetadata.resolveAddress(tokenAddress);
    const recipients = await this.getAllRecipients();
    const balances = {};
    for (const recipient of recipients) {
  ...
  async balanceOf(address) {
    const resolvedAddress = await contractMetadata.resolveAddress(address);
  ...
  async balanceOfToken(walletAddress, tokenAddress) {
    const resolvedToken = await contractMetadata.resolveAddress(tokenAddress);
    const resolvedWallet = await contractMetadata.resolveAddress(walletAddress);
    ...
    return await contractMetadata.fetchCurrencyValue(this.contractWrapper.getProvider(), resolvedToken, value);
  }
  ...
  async _pendingPayment(address, totalReceived, alreadyReleased) {
    const addressReceived = totalReceived.mul(await this.contractWrapper.readContract.shares(await contractMetadata.resolveAddress(address)));
