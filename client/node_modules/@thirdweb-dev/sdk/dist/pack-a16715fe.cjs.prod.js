...
var erc1155Standard = require('./erc-1155-standard-c3a979cb.cjs.prod.js');
...
var zod = require('zod');
var hasERC20Allowance = require('./hasERC20Allowance-07b3915e.cjs.prod.js');
require('bn.js');
...
class PackVRF {
  constructor(network, address, storage, options, chainId) {
    var _this = this;
    let contractWrapper = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : new contractMetadata.ContractWrapper(network, address, IPackVRFAbi__default["default"], options);
    QueryParams._defineProperty(this, "featureName", contractMetadata.FEATURE_PACK_VRF.name);
     ...
     * @twfeature PackVRF
     */
    QueryParams._defineProperty(this, "open", contractMetadata.buildTransactionFunction(async function (tokenId) {
     ...
     * @twfeature PackVRF
     */
    QueryParams._defineProperty(this, "claimRewards", contractMetadata.buildTransactionFunction(async function () {
    ...
    this.events = new contractMetadata.ContractEvents(this.contractWrapper);
  }
  ...
  async parseRewards(rewards) {
    ...
    for (const reward of rewards) {
      switch (reward.tokenType) {
        case 0:
          {
            const tokenMetadata = await contractMetadata.fetchCurrencyMetadata(this.contractWrapper.getProvider(), reward.assetContract);
            erc20Rewards.push({
  ...
  async canClaimRewards(claimerAddress) {
    const address = await contractMetadata.resolveAddress(claimerAddress || (await this.contractWrapper.getSignerAddress()));
  ...
  getLinkContract() {
    const linkAddress = contractMetadata.LINK_TOKEN_ADDRESS[this.chainId];
    if (!linkAddress) {
      throw new Error(`No LINK token address found for chainId ${this.chainId}`);
    }
    const contract = new contractMetadata.ContractWrapper(this.contractWrapper.getSignerOrProvider(), linkAddress, ERC20Abi__default["default"], this.contractWrapper.options);
    return new contractMetadata.Erc20(contract, this.storage, this.chainId);
  }
}
...
const CommonWrappableSchema = zod.z.object({
  contractAddress: contractMetadata.AddressOrEnsSchema
});
...
const ERC721WrappableSchema = CommonWrappableSchema.extend({
  tokenId: contractMetadata.BigNumberishSchema
});
...
const ERC1155WrappableSchema = CommonWrappableSchema.extend({
  tokenId: contractMetadata.BigNumberishSchema,
  quantity: contractMetadata.BigNumberishSchema
});
...
const ERC1155RewardSchema = ERC1155WrappableSchema.omit({
  ...
  quantityPerReward: contractMetadata.BigNumberishSchema
});
...
const ERC20RewardContentsSchema = ERC20RewardSchema.extend({
  totalRewards: contractMetadata.BigNumberishSchema.default("1")
});
...
const PackMetadataInputSchema = PackRewardsOutputSchema.extend({
  ...
  rewardsPerPack: contractMetadata.BigNumberishSchema.default("1"),
  openStartTime: contractMetadata.RawDateSchema.default(new Date())
});
...
class Pack extends erc1155Standard.StandardErc1155 {
   ...
   */
  get vrf() {
    return contractMetadata.assertEnabled(this._vrf, contractMetadata.FEATURE_PACK_VRF);
  }
  constructor(network, address, storage) {
    ...
    let contractWrapper = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : new contractMetadata.ContractWrapper(network, address, abi, options.gasless && "openzeppelin" in options.gasless ? {
      ...options,
      gasless: {
        ...options.gasless,
        openzeppelin: {
     ...
     * ```
     */
    QueryParams._defineProperty(this, "create", contractMetadata.buildTransactionFunction(async metadataWithRewards => {
     ...
     * ```
     */
    QueryParams._defineProperty(this, "addPackContents", contractMetadata.buildTransactionFunction(async (packId, packContents) => {
      const signerAddress = await this.contractWrapper.getSignerAddress();
     ...
     * ```
     */
    QueryParams._defineProperty(this, "createTo", contractMetadata.buildTransactionFunction(async (to, metadataWithRewards) => {
      const uri = await contractMetadata.uploadOrExtractURI(metadataWithRewards.packMetadata, this.storage);
      ...
      return contractMetadata.Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "createPack",
        args: [contents, numOfRewardUnits, uri, parsedMetadata.openStartTime, parsedMetadata.rewardsPerPack, await contractMetadata.resolveAddress(to)],
        parse: receipt => {
        ...
        parse: async receipt => {
          ...
          for (const reward of rewards) {
            switch (reward.tokenType) {
              case 0:
                {
                  const tokenMetadata = await contractMetadata.fetchCurrencyMetadata(_this.contractWrapper.getProvider(), reward.assetContract);
                  erc20Rewards.push({
                    contractAddress: reward.assetContract,
                    quantityPerReward: ethers.ethers.utils.formatUnits(reward.totalAmount, tokenMetadata.decimals).toString()
                  });
    ...
    this.abi = contractMetadata.AbiSchema.parse(abi || []);
    this.metadata = new contractMetadata.ContractMetadata(this.contractWrapper, contractMetadata.PackContractSchema, this.storage);
    this.app = new contractMetadata.ContractAppURI(this.contractWrapper, this.metadata, this.storage);
    this.roles = new contractMetadata.ContractRoles(this.contractWrapper, Pack.contractRoles);
    this.royalties = new contractMetadata.ContractRoyalty(this.contractWrapper, this.metadata);
    this.encoder = new contractMetadata.ContractEncoder(this.contractWrapper);
    this.estimator = new contractMetadata.GasCostEstimator(this.contractWrapper);
    this.events = new contractMetadata.ContractEvents(this.contractWrapper);
    this.interceptor = new contractMetadata.ContractInterceptor(this.contractWrapper);
    this.owner = new contractMetadata.ContractOwner(this.contractWrapper);
  ...
  async isTransferRestricted() {
    const anyoneCanTransfer = await this.contractWrapper.readContract.hasRole(contractMetadata.getRoleHash("transfer"), ethers.ethers.constants.AddressZero);
  ...
  async toPackContentArgs(metadataWithRewards) {
    ...
    for (const erc20 of erc20Rewards) {
      const normalizedQuantity = await contractMetadata.normalizePriceValue(provider, erc20.quantityPerReward, erc20.contractAddress);
    ...
    for (const erc721 of erc721Rewards) {
      const isApproved = await contractMetadata.isTokenApprovedForTransfer(this.contractWrapper.getProvider(), this.getAddress(), erc721.contractAddress, erc721.tokenId, owner);
      if (!isApproved) {
    ...
    for (const erc1155 of erc1155Rewards) {
      const isApproved = await contractMetadata.isTokenApprovedForTransfer(this.contractWrapper.getProvider(), this.getAddress(), erc1155.contractAddress, erc1155.tokenId, owner);
      if (!isApproved) {
  ...
  detectVrf() {
    if (contractMetadata.detectContractFeature(this.contractWrapper, "PackVRF")) {
