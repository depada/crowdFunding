...
class Multiwrap extends erc721Standard.StandardErc721 {
  ...
  constructor(network, address, storage) {
     ...
     */
    QueryParams._defineProperty(this, "wrap", contractMetadata.buildTransactionFunction(async (contents, wrappedTokenMetadata, recipientAddress) => {
      const uri = await contractMetadata.uploadOrExtractURI(wrappedTokenMetadata, this.storage);
      const recipient = await contractMetadata.resolveAddress(recipientAddress ? recipientAddress : await this.contractWrapper.getSignerAddress());
     ...
     */
    QueryParams._defineProperty(this, "unwrap", contractMetadata.buildTransactionFunction(async (wrappedTokenId, recipientAddress) => {
      const recipient = await contractMetadata.resolveAddress(recipientAddress ? recipientAddress : await this.contractWrapper.getSignerAddress());
      return contractMetadata.Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "unwrap",
    ...
    this.metadata = new contractMetadata.ContractMetadata(this.contractWrapper, contractMetadata.MultiwrapContractSchema, this.storage);
    this.app = new contractMetadata.ContractAppURI(this.contractWrapper, this.metadata, this.storage);
    this.roles = new contractMetadata.ContractRoles(this.contractWrapper, Multiwrap.contractRoles);
  ...
  async getWrappedContents(wrappedTokenId) {
    ...
    for (const token of wrappedTokens) {
      switch (token.tokenType) {
        case 0:
          {
            const tokenMetadata = await contractMetadata.fetchCurrencyMetadata(this.contractWrapper.getProvider(), token.assetContract);
            erc20Tokens.push({
  ...
  async toTokenStructList(contents) {
    ...
    if (contents.erc20Tokens) {
      for (const erc20 of contents.erc20Tokens) {
        const normalizedQuantity = await contractMetadata.normalizePriceValue(provider, erc20.quantity, erc20.contractAddress);
