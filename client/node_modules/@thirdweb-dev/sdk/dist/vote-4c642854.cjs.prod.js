...
class Vote {
  ...
  constructor(network, address, storage) {
     ...
     */
    QueryParams._defineProperty(this, "propose", contractMetadata.buildTransactionFunction(async (description, executions) => {
      if (!executions) {
        executions = [{
     ...
     */
    QueryParams._defineProperty(this, "vote", contractMetadata.buildTransactionFunction(async function (proposalId, voteType) {
     ...
     */
    QueryParams._defineProperty(this, "execute", contractMetadata.buildTransactionFunction(async proposalId => {
    ...
    this.metadata = new contractMetadata.ContractMetadata(this.contractWrapper, contractMetadata.VoteContractSchema, this.storage);
  ...
  async hasVoted(proposalId, account) {
    ...
    return this.contractWrapper.readContract.hasVoted(proposalId, await contractMetadata.resolveAddress(account));
  }
  ...
  async balanceOfToken(tokenAddress) {
    const erc20 = new ethers.Contract(await contractMetadata.resolveAddress(tokenAddress), ERC20Abi__default["default"], this.contractWrapper.getProvider());
    return await contractMetadata.fetchCurrencyValue(this.contractWrapper.getProvider(), tokenAddress, await erc20.balanceOf(this.contractWrapper.readContract.address));
  }
  ...
  async settings() {
    ...
    const votingTokenMetadata = await contractMetadata.fetchCurrencyMetadata(this.contractWrapper.getProvider(), votingTokenAddress);
    return {
