...
var contractMetadata = require('./contract-metadata-0a668508.cjs.prod.js');
...
class MarketplaceDirect {
  constructor(contractWrapper, storage) {
     ...
     * ```
     */
    QueryParams._defineProperty(this, "createListing", contractMetadata.buildTransactionFunction(async listing => {
      contractMetadata.validateNewListingParam(listing);
      const resolvedAssetAddress = await contractMetadata.resolveAddress(listing.assetContractAddress);
      const resolvedCurrencyAddress = await contractMetadata.resolveAddress(listing.currencyContractAddress);
      await contractMetadata.handleTokenApproval(this.contractWrapper, this.getAddress(), resolvedAssetAddress, listing.tokenId, await this.contractWrapper.getSignerAddress());
      const normalizedPricePerToken = await contractMetadata.normalizePriceValue(this.contractWrapper.getProvider(), listing.buyoutPricePerToken, resolvedCurrencyAddress);
      ...
      return contractMetadata.Transaction.fromContractWrapper({
        ...
        args: [{
          ...
          currencyToAccept: contractMetadata.cleanCurrencyAddress(resolvedCurrencyAddress),
     ...
     * ```
     */
    QueryParams._defineProperty(this, "createListingsBatch", contractMetadata.buildTransactionFunction(async listings => {
     ...
     * ```
     */
    QueryParams._defineProperty(this, "makeOffer", contractMetadata.buildTransactionFunction(async (listingId, quantityDesired, currencyContractAddress, pricePerToken, expirationDate) => {
      if (contractMetadata.isNativeToken(currencyContractAddress)) {
        throw new Error("You must use the wrapped native token address when making an offer with a native token");
      }
      const normalizedPrice = await contractMetadata.normalizePriceValue(this.contractWrapper.getProvider(), pricePerToken, currencyContractAddress);
      try {
      ...
      await contractMetadata.setErc20Allowance(this.contractWrapper, value, currencyContractAddress, overrides);
     ...
     * ```
     */
    QueryParams._defineProperty(this, "acceptOffer", contractMetadata.buildTransactionFunction(async (listingId, addressOfOfferor) => {
      /**
       * TODO:
       * - Provide better error handling if offer is too low.
       */
      await this.validateListing(ethers.BigNumber.from(listingId));
      const resolvedAddress = await contractMetadata.resolveAddress(addressOfOfferor);
     ...
     */
    QueryParams._defineProperty(this, "buyoutListing", contractMetadata.buildTransactionFunction(async (listingId, quantityDesired, receiver) => {
      ...
      const overrides = (await this.contractWrapper.getCallOverrides()) || {};
      await contractMetadata.setErc20Allowance(this.contractWrapper, value, listing.currencyContractAddress, overrides);
      return contractMetadata.Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "buy",
     ...
     */
    QueryParams._defineProperty(this, "updateListing", contractMetadata.buildTransactionFunction(async listing => {
      return contractMetadata.Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "updateListing",
        args: [listing.id, listing.quantity, listing.buyoutPrice,
        // reserve price, doesn't matter for direct listing
        listing.buyoutPrice, await contractMetadata.resolveAddress(listing.currencyContractAddress), listing.startTimeInSeconds, listing.secondsUntilEnd]
      });
    }));
    /**
     ...
     * ```
     */
    QueryParams._defineProperty(this, "cancelListing", contractMetadata.buildTransactionFunction(async listingId => {
      return contractMetadata.Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
  ...
  async getListing(listingId) {
    ...
    if (listing.assetContract === ethers.constants.AddressZero) {
      throw new contractMetadata.ListingNotFoundError(this.getAddress(), listingId.toString());
    }
    if (listing.listingType !== ListingType.Direct) {
      throw new contractMetadata.WrongListingTypeError(this.getAddress(), listingId.toString(), "Auction", "Direct");
    }
  ...
  async getActiveOffer(listingId, address) {
    ...
    const offers = await this.contractWrapper.readContract.offers(listingId, await contractMetadata.resolveAddress(address));
    if (offers.offeror === ethers.constants.AddressZero) {
      return undefined;
    }
    return await contractMetadata.mapOffer(this.contractWrapper.getProvider(), ethers.BigNumber.from(listingId), offers);
  }
  /** ******************************
   * PRIVATE FUNCTIONS
   *******************************/
  ...
  async mapListing(listing) {
    return {
      ...
      buyoutCurrencyValuePerToken: await contractMetadata.fetchCurrencyValue(this.contractWrapper.getProvider(), listing.currency, listing.buyoutPricePerToken),
      id: listing.listingId.toString(),
      tokenId: listing.tokenId,
      quantity: listing.quantity,
      startTimeInSeconds: listing.startTime,
      asset: await contractMetadata.fetchTokenMetadataForContract(listing.assetContract, this.contractWrapper.getProvider(), listing.tokenId, this.storage),
  ...
  async isStillValidListing(listing, quantity) {
    const approved = await contractMetadata.isTokenApprovedForTransfer(this.contractWrapper.getProvider(), this.getAddress(), listing.assetContractAddress, listing.tokenId, listing.sellerAddress);
    if (!approved) {
      return {
        valid: false,
    ...
    const isERC721 = await erc165.supportsInterface(contractMetadata.InterfaceId_IERC721);
    const isERC1155 = await erc165.supportsInterface(contractMetadata.InterfaceId_IERC1155);
    if (isERC721) {
...
class MarketplaceAuction {
  constructor(contractWrapper, storage) {
    ...
    QueryParams._defineProperty(this, "createListing", contractMetadata.buildTransactionFunction(async listing => {
      ...
      await contractMetadata.handleTokenApproval(this.contractWrapper, this.getAddress(), resolvedAssetAddress, listing.tokenId, await this.contractWrapper.getSignerAddress());
      const normalizedPricePerToken = await contractMetadata.normalizePriceValue(this.contractWrapper.getProvider(), listing.buyoutPricePerToken, resolvedCurrencyAddress);
      const normalizedReservePrice = await contractMetadata.normalizePriceValue(this.contractWrapper.getProvider(), listing.reservePricePerToken, resolvedCurrencyAddress);
      let listingStartTime = Math.floor(listing.startTimestamp.getTime() / 1000);
      const block = await this.contractWrapper.getProvider().getBlock("latest");
      const blockTime = block.timestamp;
      if (listingStartTime < blockTime) {
     ...
     * ```
     */
    QueryParams._defineProperty(this, "buyoutListing", contractMetadata.buildTransactionFunction(async listingId => {
      const listing = await this.validateListing(ethers.BigNumber.from(listingId));
      const currencyMetadata = await contractMetadata.fetchCurrencyMetadata(this.contractWrapper.getProvider(), listing.currencyContractAddress);
     ...
     * ```
     */
    QueryParams._defineProperty(this, "makeBid", contractMetadata.buildTransactionFunction(async (listingId, pricePerToken) => {
      const listing = await this.validateListing(ethers.BigNumber.from(listingId));
      const normalizedPrice = await contractMetadata.normalizePriceValue(this.contractWrapper.getProvider(), pricePerToken, listing.currencyContractAddress);
      ...
      if (winningBid) {
        const isWinner = contractMetadata.isWinningBid(winningBid.pricePerToken, normalizedPrice, bidBuffer);
    ...
    QueryParams._defineProperty(this, "cancelListing", contractMetadata.buildTransactionFunction(async listingId => {
      ...
      if (now.gt(startTime) && offers.offeror !== ethers.constants.AddressZero) {
        throw new contractMetadata.AuctionAlreadyStartedError(listingId.toString());
      }
      return contractMetadata.Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "closeAuction",
     ...
     */
    QueryParams._defineProperty(this, "closeListing", contractMetadata.buildTransactionFunction(async (listingId, closeFor) => {
      if (!closeFor) {
      ...
      try {
        ...
        if (err.message.includes("cannot close auction before it has ended")) {
          throw new contractMetadata.AuctionHasNotEndedError(listingId.toString(), listing.endTimeInEpochSeconds.toString());
        } else {
          throw err;
        }
      }
     ...
     * @param listingId - the auction  listing ud to close
     */
    QueryParams._defineProperty(this, "executeSale", contractMetadata.buildTransactionFunction(async listingId => {
      const listing = await this.validateListing(ethers.BigNumber.from(listingId));
      try {
        const winningBid = await this.getWinningBid(listingId);
    ...
    this.contractWrapper = contractWrapper;
    this.storage = storage;
    this.encoder = new contractMetadata.ContractEncoder(contractWrapper);
  }
  getAddress() {
    return this.contractWrapper.readContract.address;
  }
  ...
  async getListing(listingId) {
    ...
    if (listing.listingType !== ListingType.Auction) {
      throw new contractMetadata.WrongListingTypeError(this.getAddress(), listingId.toString(), "Direct", "Auction");
    }
    return await this.mapListing(listing);
  }
  ...
  async getMinimumNextBid(listingId) {
    ...
    return contractMetadata.fetchCurrencyValue(this.contractWrapper.getProvider(), listing.currencyContractAddress, minimumNextBid);
  }
  ...
  async mapListing(listing) {
    return {
      ...
      asset: await contractMetadata.fetchTokenMetadataForContract(listing.assetContract, this.contractWrapper.getProvider(), listing.tokenId, this.storage),
      reservePriceCurrencyValuePerToken: await contractMetadata.fetchCurrencyValue(this.contractWrapper.getProvider(), listing.currency, listing.reservePricePerToken),
