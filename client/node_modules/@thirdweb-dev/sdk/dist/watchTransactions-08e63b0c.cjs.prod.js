...
async function getContractFromAbi(params) {
  const resolvedAddress = await contractMetadata.resolveAddress(params.address);
  const [signer, provider] = contractMetadata.getSignerAndProvider(params.network, params.sdkOptions);
  ...
  const contract = new contractMetadata.SmartContract(signer || provider, resolvedAddress, await contractMetadata.getCompositePluginABI(resolvedAddress, contractMetadata.AbiSchema.parse(parsedAbi), provider, params.sdkOptions, getCachedStorage(params.storage)), getCachedStorage(params.storage), params.sdkOptions, chainId);
...
async function resolveContractType(params) {
  try {
    ...
    .replace(/\x00/g, "");
    return contractMetadata.getContractTypeForRemoteName(remoteContractType);
  } catch (err) {
    return "custom";
  }
}
...
async function getContract(params) {
  ...
  if (!params.contractTypeOrAbi || params.contractTypeOrAbi === "custom") {
    ...
    if (contractType === "custom") {
      const publisher = new contractMetadata.ContractPublisher(params.network, params.sdkOptions, getCachedStorage(params.storage));
      try {
      ...
      const abi = await contractMetadata.PREBUILT_CONTRACTS_MAP[contractType].getAbi(resolvedAddress, provider, getCachedStorage(params.storage));
      return getContractFromAbi({
        ...params,
        address: resolvedAddress,
        abi
  ...
  } else if (typeof params.contractTypeOrAbi === "string" && params.contractTypeOrAbi in contractMetadata.PREBUILT_CONTRACTS_MAP) {
    const contract = await contractMetadata.PREBUILT_CONTRACTS_MAP[params.contractTypeOrAbi].initialize(signer || provider, resolvedAddress, getCachedStorage(params.storage), params.sdkOptions);
    cacheContract(contract, resolvedAddress, chainId);
    return contract;
  } else {
    return getContractFromAbi({
...
async function getChainId(params) {
  const [, provider] = contractMetadata.getSignerAndProvider(params.network, params.sdkOptions);
  let chainIdPromise;
