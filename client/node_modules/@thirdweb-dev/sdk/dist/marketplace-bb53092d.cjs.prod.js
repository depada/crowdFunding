...
var marketplaceAuction = require('./marketplace-auction-ecc37d2e.cjs.prod.js');
...
class Marketplace {
  ...
  constructor(network, address, storage) {
    ...
    let contractWrapper = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : new contractMetadata.ContractWrapper(network, address, abi, options);
     ...
     * ```
     */
    QueryParams._defineProperty(this, "makeOffer", contractMetadata.buildTransactionFunction(async (listingId, pricePerToken, quantity) => {
      const listing = await this.contractWrapper.readContract.listings(listingId);
      if (listing.listingId.toString() !== listingId.toString()) {
        throw new contractMetadata.ListingNotFoundError(this.getAddress(), listingId.toString());
      }
        ...
        case marketplaceAuction.ListingType.Direct:
          {
            ...
            return await this.direct.makeOffer.prepare(listingId, quantity, contractMetadata.isNativeToken(listing.currency) ? contractMetadata.NATIVE_TOKENS[chainId].wrapped.address : listing.currency, pricePerToken);
          }
        case marketplaceAuction.ListingType.Auction:
          {
     ...
     */
    QueryParams._defineProperty(this, "setBidBufferBps", contractMetadata.buildTransactionFunction(async bufferBps => {
     ...
     */
    QueryParams._defineProperty(this, "setTimeBufferInSeconds", contractMetadata.buildTransactionFunction(async bufferInSeconds => {
      await this.roles.verify(["admin"], await this.contractWrapper.getSignerAddress());
     ...
     */
    QueryParams._defineProperty(this, "allowListingFromSpecificAssetOnly", contractMetadata.buildTransactionFunction(async contractAddress => {
      const encoded = [];
      const members = await this.roles.get("asset");
      if (members.includes(ethers.constants.AddressZero)) {
        encoded.push(this.encoder.encode("revokeRole", [contractMetadata.getRoleHash("asset"), ethers.constants.AddressZero]));
      }
      encoded.push(this.encoder.encode("grantRole", [contractMetadata.getRoleHash("asset"), contractAddress]));
      return contractMetadata.Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "multicall",
        args: [encoded]
     ...
     */
    QueryParams._defineProperty(this, "allowListingFromAnyAsset", contractMetadata.buildTransactionFunction(async () => {
      const encoded = [];
      const members = await this.roles.get("asset");
      for (const addr in members) {
        encoded.push(this.encoder.encode("revokeRole", [contractMetadata.getRoleHash("asset"), addr]));
      }
      encoded.push(this.encoder.encode("grantRole", [contractMetadata.getRoleHash("asset"), ethers.constants.AddressZero]));
      return contractMetadata.Transaction.fromContractWrapper({
        contractWrapper: this.contractWrapper,
        method: "multicall",
        args: [encoded]
    ...
    this.metadata = new contractMetadata.ContractMetadata(this.contractWrapper, contractMetadata.MarketplaceContractSchema, this.storage);
    this.app = new contractMetadata.ContractAppURI(this.contractWrapper, this.metadata, this.storage);
    this.roles = new contractMetadata.ContractRoles(this.contractWrapper, Marketplace.contractRoles);
    ...
    this.platformFees = new contractMetadata.ContractPlatformFee(this.contractWrapper);
  ...
  async isRestrictedToListerRoleOnly() {
    const anyoneCanList = await this.contractWrapper.readContract.hasRole(contractMetadata.getRoleHash("lister"), ethers.constants.AddressZero);
  ...
  async getOffers(listingId) {
    ...
    return await Promise.all(listingEvents.map(async e => {
      return await contractMetadata.mapOffer(this.contractWrapper.getProvider(), ethers.BigNumber.from(listingId), {
  ...
  async getAllListingsNoFilter(filterInvalidListings) {
    const listings = await Promise.all(Array.from(Array((await this.contractWrapper.readContract.totalListings()).toNumber()).keys()).map(async i => {
      ...
      try {
        ...
        if (err instanceof contractMetadata.ListingNotFoundError) {
          return undefined;
        } else {
