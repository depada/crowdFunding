...
var zk = require('zksync-web3');
var contractMetadata = require('../../../dist/contract-metadata-0a668508.cjs.prod.js');
...
var zk__namespace = /*#__PURE__*/_interopNamespace(zk);
...
async function zkDeployContractFromUri(publishMetadataUri, constructorParamValues, signer, storage, chainId, options) {
  const {
    compilerMetadata,
    extendedMetadata
  } = await contractMetadata.fetchAndCacheDeployMetadata(publishMetadataUri, storage);
  ...
  if (extendedMetadata && extendedMetadata.factoryDeploymentData && (extendedMetadata.isDeployableViaProxy || extendedMetadata.isDeployableViaFactory) && !forceDirectDeploy) {
    ...
    const initializerParamTypes = contractMetadata.extractFunctionParamsFromAbi(compilerMetadata.abi, extendedMetadata.factoryDeploymentData.implementationInitializerFunction).map(p => p.type);
    const paramValues = contractMetadata.convertParamValues(initializerParamTypes, constructorParamValues);
    ...
    const proxyFactory = new zk__namespace.ContractFactory(twProxyArtifactZK.abi, twProxyArtifactZK.bytecode, signer, "create");
    ...
    const constructorParamTypes = contractMetadata.extractConstructorParamsFromAbi(compilerMetadata.abi).map(p => p.type);
    const paramValues = contractMetadata.convertParamValues(constructorParamTypes, constructorParamValues);
    const factory = new zk__namespace.ContractFactory(compilerMetadata.abi, compilerMetadata.bytecode, signer, "create");
...
async function registerContractOnMultiChainRegistry(address, chainId, metadataURI) {
  try {
    ...
    const sdk = contractMetadata.ThirdwebSDK.fromPrivateKey(wallet.privateKey, "polygon", {
      gasless: {
        openzeppelin: {
